...
{{ agent_name }} = {{ class_name }}(
    model=model,
    tools=[{% for tool_name in tools if tool_name != "final_answer" %}{{ tool_name }}{% if not loop.last %}, {% endif %}{% endfor %}],
    managed_agents=[{% for subagent_name in managed_agents %}agent_{{ subagent_name }}{% if not loop.last %}, {% endif %}{% endfor %}],
